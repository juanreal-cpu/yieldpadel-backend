  ...
  <script>
    ...
    async function refreshData() {
      try {
        ...
        const res = await fetch(`${API_BASE}/api/v1/slots/?only_available=false`);
    ...
    function renderSlots() {
      ...
      countEl.textContent = `${filtered.length} turno${filtered.length === 1 ? '' : 's'}`;
      ...
      if (filtered.length === 0) {
        grid.innerHTML = `
          <div style="grid-column: 1/-1; background: #161F30; border: 1px dashed #334155; border-radius: 12px; padding: 2.5rem; text-align: center;">
            <div style="font-size: 1rem; color: #E2E8F0; margin-bottom: 0.5rem; font-weight: 600;">No hay turnos para este filtro</div>
            <button onclick="seedDemo()" class="btn-hold" style="width: auto; margin: 0 auto;">Sembrar Turnos Demo</button>
          </div>
        `;
        return;
      }
    ...
    async function seedDemo() {
      try {
        ...
        if (res.ok) {
          addEvent('Datos Demo Creados', 'Cancha y turnos generados');
          await refreshData();
        }
      } catch (e) {
        console.error(e);
