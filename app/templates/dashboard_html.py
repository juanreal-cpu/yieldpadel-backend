...
<head>
  ...
  <!-- Tailwind CSS CDN Oficial & Configuración Segura -->
  ...
  <div id="app-container" class="saas-layout flex w-full min-h-screen bg-slate-50">
    ...
    <aside class="saas-sidebar w-64 min-w-[16rem] h-screen fixed top-0 left-0 bg-white border-r border-slate-200 flex flex-col justify-between z-50 shadow-sm">
      <div>
        ...
        <!-- Menú Vertical de Navegación -->
        <nav class="space-y-1" id="main-sidebar-nav">
          <button class="sidebar-nav-item w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-semibold bg-slate-100 text-slate-900 transition active" id="tab-calendar" onclick="switchMainView('view-calendar')">
            <span class="text-base">📅</span>
            <span>Matriz Calendario</span>
          </button>
          <button class="sidebar-nav-item w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-medium text-slate-600 hover:bg-slate-50 transition" id="tab-tournaments" onclick="switchMainView('view-tournaments')">
            <span class="text-base">🏆</span>
            <span>Torneos Americanos</span>
          </button>
          <button class="sidebar-nav-item w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-medium text-slate-600 hover:bg-slate-50 transition" id="tab-crm" onclick="switchMainView('view-crm')">
            <span class="text-base">👥</span>
            <span>CRM Jugadores</span>
          </button>
          <button class="sidebar-nav-item w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-medium text-slate-600 hover:bg-slate-50 transition" id="tab-radar" onclick="switchMainView('view-radar')">
            <span class="text-base">📈</span>
            <span>Radar de Precios</span>
          </button>
          <button class="sidebar-nav-item w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-medium text-slate-600 hover:bg-slate-50 transition" id="tab-yield" onclick="switchMainView('view-yield')">
            <span class="text-base">💡</span>
            <span>Motor de Yield</span>
          </button>
          <button class="sidebar-nav-item w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-medium text-slate-600 hover:bg-slate-50 transition" id="tab-config" onclick="switchMainView('view-config')">
            <span class="text-base">⚙️</span>
            <span>Configuración del Club</span>
          </button>
          <button class="sidebar-nav-item w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-medium text-slate-600 hover:bg-slate-50 transition" id="tab-audit" onclick="switchMainView('view-audit')">
            <span class="text-base">🛡️</span>
    ...
    <div class="saas-main-content ml-64 flex-1 min-h-screen flex flex-col bg-slate-50" style="background-color: #f8fafc;">
      ...
      <div class="saas-work-area">
        ...
        <!-- ======================================================== -->
        <div id="view-calendar" class="modular-view">
          
          <!-- BARRA DE HERRAMIENTAS OPERATIVA (Card Blanca 2 Filas) -->
<!-- ======================================================== -->
          <!-- CABECERA PROFESIONAL SAAS: SEPARACIÓN EN 2 NIVELES LIMPIOS -->
          <!-- ======================================================== -->
          <div class="mb-6 rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            
            <!-- NIVEL 1: Header de Acciones Principales (bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between) -->
            <div class="bg-white border-b border-slate-200 px-6 py-4 flex flex-wrap items-center justify-between gap-4">
              <!-- Izquierda: Navegación Temporal y Fecha -->
              <div class="flex items-center gap-3">
                <div class="flex items-center rounded-lg border border-slate-200 bg-white p-1 shadow-sm">
                  <button id="btn-prev-day" onclick="shiftDay(-1)" class="p-1.5 hover:bg-slate-100 rounded text-slate-600 transition" title="Día anterior">‹</button>
                  <button id="btn-today" onclick="goToToday()" class="px-3 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-100 rounded transition" title="Ir a hoy">Hoy</button>
                  <button id="btn-next-day" onclick="shiftDay(1)" class="p-1.5 hover:bg-slate-100 rounded text-slate-600 transition" title="Día siguiente">›</button>
                </div>
                <input type="date" id="selected-date" onchange="onDateInputChange(this.value)" class="h-9 px-3 text-sm border border-slate-200 rounded-lg shadow-sm text-slate-700 font-medium focus:ring-2 focus:ring-emerald-500 focus:outline-none">
              </div>

              <!-- Derecha: Botones de Acción SaaS Agrupados -->
              <div class="flex items-center gap-2 flex-wrap">
                <button type="button" onclick="openCreateAmericanoModal()" id="btn-create-americano" class="bg-violet-600 hover:bg-violet-700 text-white text-xs font-semibold px-3.5 py-2 rounded-lg shadow-sm flex items-center gap-1.5 transition">
                  🏆 + Torneo Americano
                </button>
                <button type="button" onclick="broadcastAvailability()" id="btn-broadcast-avail" class="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold px-3.5 py-2 rounded-lg shadow-sm flex items-center gap-1.5 transition">
                  📢 Difundir WhatsApp
                </button>
                <button type="button" onclick="broadcastPromoUrgent()" id="btn-broadcast-promo" class="border border-rose-300 bg-rose-50 text-rose-700 text-xs font-semibold px-3.5 py-2 rounded-lg hover:bg-rose-100 transition shadow-xs flex items-center gap-1.5">
                  ⚡ Remate Flash
                </button>
                <button type="button" onclick="seedFiveCourts()" id="btn-seed-courts" class="border border-slate-200 bg-white text-slate-700 text-xs font-semibold px-3.5 py-2 rounded-lg hover:bg-slate-50 transition shadow-xs flex items-center gap-1.5">
                  🌱 Sembrar 7 Días
                </button>
                <div id="active-user-badge" class="ml-2 flex items-center gap-2 px-3 py-1.5 bg-slate-100 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 shadow-xs" title="Sesión activa">
            ...
            <!-- NIVEL 2: Filtros y Deporte (bg-slate-50 border-b border-slate-200 px-6 py-3 flex flex-wrap items-center justify-between gap-4) -->
            <div class="bg-slate-50 border-b border-slate-200 px-6 py-3 flex flex-wrap items-center justify-between gap-4">
              <!-- Segmented Control de Deportes (Tabs tipo pastilla) -->
              <div class="flex items-center gap-1.5" id="sport-selector-container">
                <button type="button" onclick="setSportFilter('PADEL')" id="btn-sport-padel" class="bg-slate-900 text-white rounded-lg px-3.5 py-1.5 text-xs font-medium flex items-center gap-1.5 shadow-sm transition active">
                  🎾 Pádel
                </button>
                <button type="button" onclick="setSportFilter('PICKLEBALL')" id="btn-sport-pickleball" class="bg-white border border-slate-200 text-slate-600 hover:bg-slate-100 px-3.5 py-1.5 rounded-lg text-xs font-medium flex items-center gap-1.5 transition">
                  🏓 Pickleball
                </button>
                <button type="button" onclick="setSportFilter('VOLLEYBALL')" id="btn-sport-volleyball" class="bg-white border border-slate-200 text-slate-600 hover:bg-slate-100 px-3.5 py-1.5 rounded-lg text-xs font-medium flex items-center gap-1.5 transition">
                  🏐 Vóley
                </button>
                <button type="button" onclick="setSportFilter('PILATES')" id="btn-sport-pilates" class="bg-white border border-slate-200 text-slate-600 hover:bg-slate-100 px-3.5 py-1.5 rounded-lg text-xs font-medium flex items-center gap-1.5 transition">
                  🧘 Pilates
                </button>
              </div>

              <!-- Filtro Horario Horizontal Compacto -->
              <div class="flex items-center gap-1 bg-white p-1 rounded-lg border border-slate-200 shadow-sm" id="time-filter-container">
                <button type="button" onclick="setTimeFilter('ALL')" id="btn-time-all" class="px-3 py-1 text-xs font-semibold rounded bg-slate-900 text-white transition active">Todo el día</button>
                <button type="button" onclick="setTimeFilter('MORNING')" id="btn-time-morning" class="px-3 py-1 text-xs font-medium text-slate-600 hover:bg-slate-100 rounded transition">Mañana (6-12)</button>
                <button type="button" onclick="setTimeFilter('AFTERNOON')" id="btn-time-afternoon" class="px-3 py-1 text-xs font-medium text-slate-600 hover:bg-slate-100 rounded transition">Tarde (12-18)</button>
                <button type="button" onclick="setTimeFilter('NIGHT')" id="btn-time-night" class="px-3 py-1 text-xs font-medium text-slate-600 hover:bg-slate-100 rounded transition">Noche (18-24)</button>
              </div>

              <!-- Selector de Cancha a la derecha con select nativo estilizado en Tailwind -->
              <div class="flex items-center gap-2">
                <label for="court-zoom-select" class="text-xs font-medium text-slate-500">Pista:</label>
                <select id="court-zoom-select" onchange="setCourtZoom(this.value)" class="h-8 text-xs border border-slate-200 rounded-lg bg-white px-2.5 text-slate-700 font-medium focus:ring-2 focus:ring-emerald-500 focus:outline-none shadow-sm">
                  <option value="ALL">🏟️ Todas las pistas</option>
                </select>
              </div>
            </div>
        ...
        <!-- ======================================================== -->
        <div id="view-tournaments" class="modular-view hidden" style="display: none;">
        ...
        <!-- ======================================================== -->
        <div id="view-crm" class="modular-view hidden" style="display: none;">
          <div style="max-width: 1100px; margin: 0 auto; padding-bottom: 3rem;">
        ...
        <!-- ======================================================== -->
        <div id="view-radar" class="modular-view hidden" style="display: none;">
        ...
        <!-- ======================================================== -->
        <div id="view-yield" class="modular-view hidden" style="display: none;">
        ...
        <!-- ======================================================== -->
        <div id="view-config" class="modular-view hidden" style="display: none;">
          <div style="max-width: 1050px; margin: 0 auto;">
  ...
  <!-- ======================================================== -->
        ...
        <!-- ======================================================== -->
        <div id="view-audit" class="modular-view hidden" style="display: none;">
          <div style="max-width: 1200px; margin: 0 auto; padding-bottom: 3rem;">
  ...
  <script>
    ...
    function setSportFilter(sport) {
      let sUpper = (sport || 'PADEL').toUpperCase();
      if (sUpper === 'VOLEY') sUpper = 'VOLLEYBALL';
      currentSportFilter = sUpper;

      const sports = {
        PADEL: 'btn-sport-padel',
        PICKLEBALL: 'btn-sport-pickleball',
        VOLLEYBALL: 'btn-sport-volleyball',
        PILATES: 'btn-sport-pilates'
      };

      Object.entries(sports).forEach(([key, btnId]) => {
        const btn = document.getElementById(btnId);
        if (btn) {
          if (key === currentSportFilter) {
            btn.className = 'bg-slate-900 text-white rounded-lg px-3.5 py-1.5 text-xs font-medium flex items-center gap-1.5 shadow-sm transition active';
          } else {
            btn.className = 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-100 px-3.5 py-1.5 rounded-lg text-xs font-medium flex items-center gap-1.5 transition';
          }
        }
      });
    ...
    window.selectSportFilter = setSportFilter;
    ...
    window.switchMainView = function(viewId) {
      try {
        let cleanId = (viewId || 'view-calendar').toString().trim();
        if (!cleanId.startsWith('view-')) {
          cleanId = 'view-' + cleanId;
        }

        // Aliases de compatibilidad
        if (cleanId === 'view-matrix') cleanId = 'view-calendar';
        if (cleanId === 'view-indicators') cleanId = 'view-yield';
        ...
        // Actualizar clases activas en botones de sidebar (bg-slate-100 text-slate-900 font-semibold vs text-slate-600 hover:bg-slate-50)
        const navMapping = {
          'view-calendar': 'tab-calendar',
          'view-tournaments': 'tab-tournaments',
          'view-crm': 'tab-crm',
          'view-radar': 'tab-radar',
          'view-yield': 'tab-yield',
          'view-config': 'tab-config',
          'view-audit': 'tab-audit'
        };

        Object.entries(navMapping).forEach(([vId, tId]) => {
          const btn = document.getElementById(tId);
          if (btn) {
            if (vId === cleanId) {
              btn.className = 'sidebar-nav-item w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-semibold bg-slate-100 text-slate-900 transition active';
            } else {
              btn.className = 'sidebar-nav-item w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-medium text-slate-600 hover:bg-slate-50 transition';
            }
          }
        });
        ...
        if (cleanId === 'view-config' && typeof loadClubConfig === 'function') {
          ...
          loadCRMDirectory();
        } else if (cleanId === 'view-tournaments' && typeof loadTournamentsList === 'function') {
          loadTournamentsList();
        } else if (cleanId === 'view-yield' && typeof loadYieldEngineStats === 'function') {
          loadYieldEngineStats();
        } else if (cleanId === 'view-radar' && typeof initRadarMap === 'function') {
          initRadarMap();
        } else if (cleanId === 'view-audit' && typeof loadAuditData === 'function') {
          loadAuditData();
        } else if (cleanId === 'view-calendar' && typeof fetchSlots === 'function') {
          fetchSlots();
        }
      } catch (err) {
    ...
    function switchMainView(v) { window.switchMainView(v); }

    // Helpers de navegación temporal
    function shiftDay(delta) {
      const input = document.getElementById('selected-date');
      const cur = (input && input.value) ? input.value : (selectedDate || '2026-09-07');
    ...
    function setRelativeDate(offset) {
      if (offset === 0) goToToday();
      else shiftDay(offset);
    }
...
${data.message}`);
    ...
    function setTimeFilter(timeFilter) {
      ...
      const tods = {
        ALL: 'btn-time-all',
        MORNING: 'btn-time-morning',
        AFTERNOON: 'btn-time-afternoon',
        NIGHT: 'btn-time-night'
      };

      Object.entries(tods).forEach(([key, btnId]) => {
        const btn = document.getElementById(btnId);
        if (btn) {
          if (key === currentTimeFilter) {
            btn.className = 'px-3 py-1 text-xs font-semibold rounded bg-slate-900 text-white transition active';
          } else {
            btn.className = 'px-3 py-1 text-xs font-medium text-slate-600 hover:bg-slate-100 rounded transition';
          }
        }
      });
    ...
    window.filterTimeOfDay = setTimeFilter;
    ...
    async function fetchSlots() {
      ...
      if (container) {
        container.innerHTML = `
          <div id="loading-spinner" style="grid-column: 1/-1;" class="py-16 flex flex-col items-center justify-center text-center">
            <div class="inline-block w-9 h-9 border-3 border-slate-200 border-t-emerald-600 rounded-full animate-spin mb-3"></div>
            <div class="text-sm font-bold text-slate-800">Cargando turnos de la jornada...</div>
            <div class="text-xs text-slate-500 mt-1">Consultando disponibilidad y tarifas de ${currentSportFilter}</div>
          </div>
        `;
      }
      ...
      try {
        ...
        // Ocultar siempre el spinner
        const spinner = document.getElementById('loading-spinner') || document.getElementById('grid-loader');
        if (spinner) spinner.style.display = 'none';

        // Si slotList viene vacío, renderizar contenedor con mensaje claro y botón para sembrar turnos
        if (slotList.length === 0) {
          if (container) {
            container.style.gridTemplateColumns = '1fr';
            container.style.minWidth = '100%';
            container.innerHTML = `
              <div class="col-span-full py-16 text-center bg-white border border-slate-200 rounded-xl p-8 my-4 shadow-sm" style="grid-column: 1/-1;">
                <div class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-slate-100 text-slate-400 mb-3 text-2xl">
                  📅
                </div>
                <h3 class="text-base font-bold text-slate-800 mb-1">No hay turnos para este deporte en la fecha seleccionada</h3>
                <p class="text-xs text-slate-500 mb-4 max-w-md mx-auto">No se encontraron franjas horarias programadas para ${currentSportFilter} el ${selectedDate}. Puedes generar la grilla automáticamente con un clic.</p>
                <button type="button" onclick="seedFiveCourts()" class="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm transition">
                  <span>🌱</span> Sembrar Turnos para los Próximos 7 Días
                </button>
              </div>
            `;
          }
        ...
        try {
          ...
          console.error("Error defensivo en renderCalendarMatrix:", renderErr);
          if (container) {
            container.style.gridTemplateColumns = '1fr';
            container.style.minWidth = '100%';
            container.innerHTML = `
              <div class="col-span-full py-16 text-center bg-white border border-slate-200 rounded-xl p-8 my-4 shadow-sm" style="grid-column: 1/-1;">
                <h3 class="text-sm font-semibold text-slate-700">No hay turnos para este deporte en la fecha seleccionada.</h3>
                <button type="button" onclick="seedFiveCourts()" class="mt-3 inline-flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-semibold rounded-lg bg-emerald-600 text-white shadow-sm">
                  🌱 Sembrar Turnos
                </button>
              </div>
            `;
        ...
        if (container) {
          ...
          container.innerHTML = `
            <div class="col-span-full py-16 text-center bg-white border border-slate-200 rounded-xl p-8 my-4 shadow-sm" style="grid-column: 1/-1;">
              <div class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-rose-50 text-rose-500 mb-3 text-2xl">
                ⚠️
              </div>
              <h3 class="text-base font-bold text-slate-800 mb-1">Fallo de conexión al cargar turnos</h3>
              <p class="text-xs text-slate-500 mb-4 max-w-md mx-auto">No se pudo consultar el servidor en este momento. Verifica tu conexión o intenta sembrar los turnos.</p>
              <button type="button" onclick="fetchSlots()" class="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-lg bg-slate-900 hover:bg-slate-800 text-white shadow-sm transition">
                ↻ Reintentar Carga
              </button>
            </div>
          `;
    ...
    function initDashboard() {
      try {
        ...
        selectedDate = (picker && picker.value) ? picker.value : '2026-09-07';
        updateDateUI();
        fetchCourts().then(() => {
          fetchSlots();
        }).catch(err => {
          console.warn("fetchCourts inicial con advertencia:", err);
          fetchSlots();
        });

        // Refresco periódico pasivo cada 30 segundos
        setInterval(() => {
        ...
        console.error("Error defensivo en initDashboard:", err);
      }
    }
