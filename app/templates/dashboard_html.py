...
<body>
          ...
          <div style="max-width: 1050px; margin: 0 auto;">
            ...
            <div class="sidebar-card">
              <div class="sidebar-title">
                ...
                <span class="pro-badge">CRM Club</span>
              </div>
              <div id="crm-customers-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1rem;">
                <div style="background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 8px; padding: 1rem;">
                  <div style="font-weight: 700; color: #0F172A;">Juan Real Florez</div>
                  <div style="font-size: 0.72rem; color: #64748B;">+57 313 2058547 • 4ta Categoría</div>
                  <div style="margin-top: 0.5rem; font-size: 0.75rem; color: #059669; font-weight: 600;">Socio VIP • Cliente Fundador</div>
                </div>
                <div style="background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 8px; padding: 1rem;">
                  <div style="font-weight: 700; color: #0F172A;">Camilo Real</div>
                  <div style="font-size: 0.72rem; color: #64748B;">+57 300 1234567 • 4ta Categoría</div>
                  <div style="margin-top: 0.5rem; font-size: 0.75rem; color: #0284C7; font-weight: 600;">Estándar • Jugador Semanal</div>
                </div>
                <div style="background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 8px; padding: 1rem;">
                  <div style="font-weight: 700; color: #0F172A;">David P</div>
                  <div style="font-size: 0.72rem; color: #64748B;">+57 310 9876543 • 3ra Categoría</div>
                  <div style="margin-top: 0.5rem; font-size: 0.75rem; color: #7C3AED; font-weight: 600;">3ra Avanzada • Torneos</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <form id="reserve-block-form" onsubmit="handleReserveOrBlockSubmit(event)">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem;" id="rb-client-details-row">
          <div class="form-group">
            ...
            <input type="text" id="rb-client-category" class="form-control" placeholder="4ta" value="4ta">
          </div>
          <div class="form-group">
            <label class="form-label">Tipo de Cliente / Socio</label>
            <input type="text" id="rb-client-type" class="form-control" placeholder="Estándar" value="Estándar">
          </div>
        </div>
  ...
  <script>
    ...
    async function loadClubConfig() {
      try {
        ...
        if (res.ok) {
          ...
          document.getElementById('cfg-whatsapp-group').value = cfg.broadcast_group_id || '573132058547';
        ...
        // Render courts list in config con inputs editables y toggles activos
        ...
        if (courtListEl && allCourts.length > 0) {
          ...
          allCourts.forEach((c, idx) => {
            ...
            cHtml += `
              <div style="display: flex; align-items: center; gap: 0.75rem; background: #F8FAFC; border: 1px solid #CBD5E1; padding: 0.6rem 0.85rem; border-radius: 8px;">
                <span style="font-weight: 800; color: #475569; font-size: 0.8rem; width: 28px;">#${num}</span>
                <input type="text" class="form-control cfg-court-name" data-court-id="${c.id}" value="${c.name}" style="flex: 1; font-weight: 700; padding: 0.35rem 0.6rem; font-size: 0.82rem;" title="Nombre de la pista" />
                <label style="display: flex; align-items: center; gap: 0.35rem; font-size: 0.75rem; font-weight: 700; color: ${isAct ? '#059669' : '#64748B'}; cursor: pointer; white-space: nowrap;">
    ...
    async function saveClubConfig() {
      const courtInputs = document.querySelectorAll('.cfg-court-name');
      const courtsData = [];
      courtInputs.forEach(input => {
        const cid = input.dataset.courtId;
        const activeBox = document.querySelector(`.cfg-court-active[data-court-id="${cid}"]`);
        courtsData.push({
          id: cid,
          name: input.value.trim(),
          is_active: activeBox ? activeBox.checked : true
        });
      });

      const payload = {
        base_valle: parseFloat(document.getElementById('cfg-base-valle').value),
        base_pico: parseFloat(document.getElementById('cfg-base-pico').value),
        safety_floor: parseFloat(document.getElementById('cfg-floor-price').value),
        promo_discount_percent: parseInt(document.getElementById('cfg-discount-pct').value, 10),
        cancellation_grace_minutes: parseInt(document.getElementById('cfg-cancel-grace').value, 10),
        confirmation_grace_minutes: parseInt(document.getElementById('cfg-confirm-grace').value, 10),
        broadcast_group_id: document.getElementById('cfg-whatsapp-group').value.trim(),
        courts: courtsData
      };
      ...
      try {
        ...
        const data = await res.json();
        if (res.ok) {
          addEvent('Configuración Actualizada', 'Nuevos parámetros de Yield y nombres de pistas aplicados', 'green');
          alert('✅ ¡Configuración del club guardada exitosamente!\\n\\nLas pistas y parámetros operativos han sido actualizados.');
          if (data.courts) allCourts = data.courts;
          await fetchSlots();
        } else {
